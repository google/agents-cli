...
description: >
  ...
  Do NOT use for API code patterns (use agents-cli-adk-code), evaluation
  (use agents-cli-eval), or project scaffolding (use agents-cli-scaffold).
metadata:
  author: Google
  license: Apache-2.0
  version: 0.0.3
...
> Prefer using the `agents-cli` commands throughout this guide — they wrap Terraform, Docker, and deployment into a tested pipeline. If your project isn't scaffolded yet, see `/agents-cli-scaffold` to add deployment support first.
...
- **`cicd-pipeline.md`** — Full CI/CD pipeline setup, `infra prod` flags, runner comparison, WIF auth, pipeline stages
- **`testing-deployed-agents.md`** — Testing instructions per deployment target, curl examples, load tests

> **Observability:** See the `/agents-cli-observability` skill for Cloud Trace, prompt-response logging, BigQuery Analytics, and third-party integrations.
...
1. If prototype (no deployment target), first enhance: `agents-cli enhance . --deployment-target <target>`
...
> **Do NOT run `agents-cli infra dev` before deploying.** It is not a prerequisite. `agents-cli deploy` works on its own for all deployment targets.

### Dev Infrastructure Setup (Optional — Advanced)

`agents-cli infra dev` runs `terraform apply` in `deployment/terraform/dev/`. This is **only needed when you want to replicate production infrastructure locally** (service accounts, IAM bindings, telemetry resources, Artifact Registry). It is NOT required for deploying.

```bash
# Optional — only if you need prod-like infra in dev
agents-cli infra dev
```
...
For the full CI/CD pipeline setup guide — prerequisites, `infra prod` flags, runner comparison, WIF authentication, pipeline stages, and production approval — see `references/cicd-pipeline.md`.
...
For registering deployed agents with Gemini Enterprise, see `/agents-cli-publish`.
