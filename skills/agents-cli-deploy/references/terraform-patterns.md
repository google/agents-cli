...
> **Assumes `/agents-cli-scaffold` scaffolding.** These patterns apply to projects with `deployment/terraform/` directories.
...
agents-cli infra dev  # Runs terraform apply in deployment/terraform/dev/
...
**Cloud SQL sessions:**
- Already configured when using `--session-type cloud_sql` via the Agents CLI (see `/agents-cli-scaffold`)
...
Use the `--local-state` flag with `infra prod` to skip remote backend setup and store state locally:
