...
description: >
  ...
  Do NOT use for API code patterns (use agents-cli-adk-code), deployment
  (use agents-cli-deploy), or project scaffolding (use agents-cli-scaffold).
metadata:
  author: Google
  license: Apache-2.0
  version: 0.0.3
...
> **Scaffolded project?** If you used `/agents-cli-scaffold`, you already have `agents-cli eval`, `tests/eval/evalsets/`, and `tests/eval/eval_config.json`. Start with `agents-cli eval` and iterate from there.
...
- `/agents-cli-workflow` — Development workflow and the spec-driven build-evaluate-deploy lifecycle
...
- `/agents-cli-observability` — Cloud Trace, logging, and monitoring for debugging agent behavior
